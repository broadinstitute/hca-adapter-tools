...
def main():
    ...
    with open(analysis_files_metadata_jsons) as f:
        ...
        for file in analysis_files_metadata_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}metadata/analysis_file/'.format(file, staging_bucket),
                shell=True,
            )
    ...
    with open(analysis_process_jsons) as f:
        ...
        for file in analysis_process_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}metadata/analysis_process/'.format(
                    file, staging_bucket
                ),
                shell=True,
            )
    ...
    with open(analysis_protocol_jsons) as f:
        ...
        for file in analysis_protocol_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}metadata/analysis_protocol/'.format(
                    file, staging_bucket
                ),
                shell=True,
            )
    ...
    with open(analysis_files_descriptors_jsons) as f:
        ...
        for file in analysis_files_descriptors_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}descriptors/analysis_file/'.format(
                    file, staging_bucket
                ),
                shell=True,
            )
    ...
    with open(links_jsons) as f:
        ...
        for file in links_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}links/'.format(file, staging_bucket), shell=True
            )
    ...
    with open(data_files) as f:
        ...
        for file in data_files_list:
            subprocess.run(
                'gsutil cp {0} {1}data/'.format(file, staging_bucket), shell=True
            )
    ...
    with open(reference_metadata_jsons) as f:
        ...
        for file in reference_metadata_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}metadata/reference_file/'.format(
                    file, staging_bucket
                ),
                shell=True,
            )
    ...
    with open(reference_file_descriptor_jsons) as f:
        ...
        for file in reference_file_descriptor_jsons_list:
            subprocess.run(
                'gsutil cp {0} {1}descriptors/reference_file/'.format(
                    file, staging_bucket
                ),
                shell=True,
            )
